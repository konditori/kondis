<script lang="ts">
  ...
  const optionGroups = $derived(
    history
      ? Object.entries(
          Object.groupBy(history.options, ({ valueKind }) => valueKind),
        )
      : [],
  );
  ...
  function secondaryValue(
    effort: NonNullable<typeof history>["efforts"][number],
  ): string {
    if (!history) return "";
      ...
      <div class="distance-picker">
        ...
        >
          {#each optionGroups as [kind, options]}
            <optgroup
              label={optionGroupLabels[kind as keyof typeof optionGroupLabels]}
            >
              {#each options ?? [] as option}<option value={option.type}
                  >{bestEffortLabel(option.type)}</option
                >{/each}
            </optgroup>
          {/each}
        </select>
    ...
    <section class="podium-section">
      ...
      <div class="podium-grid">
        {#each podium as effort, index}
          ...
          >
            <span class="medal"
              ><Medal size={22} /><small>{medalNames[index]}</small></span
            >
            <div>
              <strong
                >{bestEffortValue(
              ...
              ><small>{new Date(effort.startedAt).toLocaleDateString()}</small>
            </div>
    ...
    <section class="effort-history-section">
      ...
      <div class="effort-history-list">
        {#each recentEfforts as effort}
          <a
            class="effort-history-row"
            href={`/activities/${effort.activityId}`}
          >
            <span class="history-rank" class:ranked={effort.overallRank <= 3}
              >{effort.overallRank <= 3 ? `#${effort.overallRank}` : "—"}</span
            >
            ...
            <span class="history-time"
              ><Timer size={16} /><strong
                >{bestEffortValue(
                  effort.value,
                  history.valueKind,
                  data.unitSystem,
                )}</strong
              >{#if secondaryValue(effort)}<small
                  >{secondaryValue(effort)}</small
                >{/if}</span
            >
            <span class="history-badges">
              ...
              {#if effort.yearRank <= 3}<span
                  ...
                  ><Medal size={31} /> {effort.year}</span
                >{/if}
            </span>
