<script lang="ts">
  ...
  function x(index: number): number {
    ...
    return padding.left + ((dates[index] - minDate) / (maxDate - minDate)) * (width - padding.left - padding.right);
  }
  ...
  function tooltipX(index: number): number {
    return Math.min(Math.max(x(index) - tooltipWidth / 2, padding.left), width - padding.right - tooltipWidth);
  }
...
<div class="effort-chart-wrap">
  <svg class="effort-chart" viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`${label} best efforts over time`}>
    {#each yTicks as tick}
      <line x1={padding.left} x2={width - padding.right} y1={y(tick)} y2={y(tick)} class="chart-grid" />
      <text x={padding.left - 11} y={y(tick) + 4} text-anchor="end" class="chart-label">{bestEffortValue(tick, valueKind, unitSystem)}</text>
    {/each}
    {#if efforts.length > 1}
    ...
    {#each efforts as effort, index}
      ...
      >
        <circle class="chart-hit-area" cx={x(index)} cy={y(effort.value)} r="12" />
        <circle
          class="chart-point"
          cx={x(index)}
          cy={y(effort.value)}
    ...
    {#if hoveredIndex !== null}
      ...
      <g class="chart-tooltip" transform={`translate(${tooltipX(hoveredIndex)} ${tooltipY(hoveredIndex)})`}>
        <rect width={tooltipWidth} height={tooltipHeight} rx="8" />
        <text x="12" y="22" class="chart-tooltip-time">{bestEffortValue(effort.value, valueKind, unitSystem)}</text>
        <text x="12" y="40" class="chart-tooltip-date">
          {new Date(effort.startedAt).toLocaleDateString(undefined, { day: "numeric", month: "short", year: "numeric" })}
        </text>
      </g>
    {/if}
    <text x={padding.left} y={height - 10} class="chart-label">{firstYear}</text>
    {#if lastYear !== firstYear}<text x={width - padding.right} y={height - 10} text-anchor="end" class="chart-label">{lastYear}</text>{/if}
  </svg>
</div>
