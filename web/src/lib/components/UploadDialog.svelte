<script lang="ts">
  ...
  let {
    open = $bindable(false),
    eventsUrl,
  }: { open: boolean; eventsUrl: string } = $props();
  ...
  async function addFiles(files: FileList | File[]) {
    ...
    const navigateToActivity =
      accepted.length === 1 &&
      !accepted[0]?.name.toLowerCase().endsWith(".zip");
    const activityCreated = navigateToActivity
      ? waitForActivityCreated()
      : undefined;
    ...
    >
      <header>
        <div>
          ...
          <h2 id="upload-title">Add activities</h2>
        </div>
        <button class="icon-button" onclick={close} aria-label="Close"
          ><X size={20} /></button
        >
      </header>
      ...
      <input
        ...
        class="sr-only"
        type="file"
        accept=".fit,.tcx,.gpx,.zip,application/zip,application/octet-stream,application/gpx+xml"
        multiple
          ...
          {#each uploads as item}
            <div class="upload-row">
              <span class="file-name"
                ...
                ></span
              >
              {#if item.state === "uploading"}<LoaderCircle
                  class="spin"
                  size={19}
                />
              {:else if item.state === "done"}<span class="processing"
                  ><Check class="success" size={16} />
                  {item.message ?? "Done"}</span
                >
              {:else if item.state === "error"}<span
                  class="error"
                  title={item.message}>{item.message ?? "Failed"}</span
                >
              {/if}
            </div>
          {/each}
