...
The developer documentation consists of two parts: the main markdown files for the documentation which live in `static/apps/docs.kondis.org/dev`, and then the API reference documentation autogenerated from the server code.

In the developer docs site folder, run

```bash
cd static/apps/developers.kondis.org
mise dev
```
