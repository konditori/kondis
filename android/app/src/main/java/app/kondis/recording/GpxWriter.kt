...
class GpxWriter @Inject constructor() {
    fun write(destination: File, recording: RecordingState, sport: String): File {
        require(recording.points.isNotEmpty()) { "A workout needs at least one GPS point" }
        val points = recording.points.joinToString(separator = "\n") { point ->
            val elevation = point.altitude?.let { "<ele>${"%.2f".format(java.util.Locale.US, it)}</ele>" }.orEmpty()
            "      <trkpt lat=\"${point.latitude}\" lon=\"${point.longitude}\">$elevation<time>${point.recordedAt}</time></trkpt>"
        }
