...
data class PageResult(val nextCursor: String?, val total: Int)
...
class ActivityRepository @Inject constructor(
    ...
    fun activities(search: String): Flow<List<Activity>> = activityDao
        .observeActivities(search.trim().lowercase())
        .map { rows -> rows.mapNotNull { row -> decodeActivity(row.payload) } }

    fun detail(id: String): Flow<ActivityDetail?> = activityDao.observeDetail(id)
    ...
    suspend fun loadMore(cursor: String, search: String = ""): PageResult {
    ...
    private fun toEntity(activity: Activity) = ActivityEntity(
        ...
        searchableText = listOf(activity.name, activity.description, activity.sport)
            .filterNotNull()
            .joinToString(" ")
            .lowercase(),
    ...
    private fun decodeActivity(payload: String): Activity? = runCatching {
        json.decodeFromString(Activity.serializer(), payload)
    }.getOrNull()

    private fun decodeDetail(payload: String): ActivityDetail? = runCatching {
