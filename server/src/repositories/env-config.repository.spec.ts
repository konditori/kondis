import { afterEach, beforeEach, describe, expect, it } from 'vitest';

import { EnvConfigRepository } from 'src/repositories/env-config.repository';

describe('EnvConfigRepository', () => {
  let original: NodeJS.ProcessEnv;

  beforeEach(() => {
    original = process.env;
    process.env = {
      KONDIS_DB_USERNAME: 'kondis',
      KONDIS_DB_PASSWORD: 'kondis',
      KONDIS_DB_DATABASE_NAME: 'kondis',
    };
  });

  afterEach(() => {
    process.env = original;
  });

  describe('environment cache', () => {
    it('caches the parsed environment per repository', () => {
      const repository = new EnvConfigRepository();
      const first = repository.getEnv();

      process.env.KONDIS_PORT = '8080';

      expect(repository.getEnv()).toBe(first);
      expect(repository.getEnv().port).toBe(2293);
    });

    it('uses a fresh cache for a new repository', () => {
      process.env.KONDIS_PORT = '8080';

      expect(new EnvConfigRepository().getEnv().port).toBe(8080);
    });
  });

  describe('database defaults', () => {
    it('uses the Docker Compose database service when the hostname is unset', () => {
      expect(new EnvConfigRepository().getEnv().database.host).toBe('database');
    });

    it('uses the configured database hostname when provided', () => {
      process.env.KONDIS_DB_HOSTNAME = 'localhost';

      expect(new EnvConfigRepository().getEnv().database.host).toBe('localhost');
    });

    it('rejects an invalid database port', () => {
      process.env.KONDIS_DB_PORT = '5432.5';

      expect(() => new EnvConfigRepository().getEnv()).toThrow(/KONDIS_DB_PORT must be a positive integer/);
    });

    it('rejects an invalid server port', () => {
      process.env.KONDIS_PORT = 'not-a-port';

      expect(() => new EnvConfigRepository().getEnv()).toThrow(/KONDIS_PORT must be a positive integer/);
    });
  });

  describe('server defaults', () => {
    it('uses local deployment by default', () => {
      expect(new EnvConfigRepository().deployTarget).toBe('local');
    });

    it('reads the configured deployment target', () => {
      process.env.KONDIS_DEPLOY_TARGET = 'cloudflare';

      expect(new EnvConfigRepository().deployTarget).toBe('cloudflare');
    });

    it('rejects an invalid deployment target', () => {
      process.env.KONDIS_DEPLOY_TARGET = 'worker';

      expect(() => new EnvConfigRepository().deployTarget).toThrow(/KONDIS_DEPLOY_TARGET must be local or cloudflare/);
    });

    it('does not trust client-controlled proxy headers by default', () => {
      expect(new EnvConfigRepository().trustProxyHeaders).toBe(false);
    });

    it('listens on all interfaces by default', () => {
      expect(new EnvConfigRepository().getEnv().listenAddress).toBe('0.0.0.0');
    });

    it('uses the configured listen address when provided', () => {
      process.env.KONDIS_LISTEN_ADDRESS = '127.0.0.1';

      expect(new EnvConfigRepository().getEnv().listenAddress).toBe('127.0.0.1');
    });

    it('disables demo mode by default', () => {
      expect(new EnvConfigRepository().demoMode).toBe(false);
    });

    it('reads demo mode from the environment', () => {
      process.env.KONDIS_DEMO_MODE = 'true';

      expect(new EnvConfigRepository().demoMode).toBe(true);
      expect(new EnvConfigRepository().getEnv().demoMode).toBe(true);
    });

    it('rejects an invalid demo mode value', () => {
      process.env.KONDIS_DEMO_MODE = 'yes';

      expect(() => new EnvConfigRepository().demoMode).toThrow(/KONDIS_DEMO_MODE must be true or false/);
    });
  });
});
