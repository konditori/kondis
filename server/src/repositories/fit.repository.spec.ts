...
const makeRepository = () => new FitRepository(new ConsoleLogger({ logLevels: [] }));

const decode = (contents: Buffer) => makeRepository().decode(contents);

describe('FitRepository', () => {
  describe('decode', () => {
    it('exposes session fields under their camelCase names', () => {
      const session = decode(buildFitFile()).sessionMesgs?.[0];
    ...
    it('exposes record fields under their camelCase names', () => {
      const records = decode(buildFitFile()).recordMesgs;
    ...
    it('exposes lap fields under their camelCase names', () => {
      const laps = decode(buildFitFile()).lapMesgs;
    ...
    it('reports position in degrees rather than semicircles', () => {
      const record = decode(buildFitFile()).recordMesgs?.[0];
    ...
    it('keeps the readable part of a file with a corrupt trailing CRC', () => {
      ...
      expect(decode(corrupted).recordMesgs).toHaveLength(1);
    });
    ...
    it.each([
      ...
      expect(() => decode(contents)).toThrow(FitDecodeError);
      expect(() => decode(contents)).toThrow(/not a valid FIT file/);
    });
  });
});
