...
import { FileSizeLimitError, type StorageFile, StorageRepository } from 'src/contracts/storage.repository';
...
export class FileSystemStorageRepository extends StorageRepository {
  constructor(
    private readonly config: EnvConfigRepository,
    private readonly crypto: NodeCryptoRepository,
  ) {
    super();
  }
  ...
  importFile = async (sourcePath: string, relativePath: string): Promise<void> => {
    const target = this.absolutePath(relativePath);
    await mkdir(dirname(target), { recursive: true });
    try {
  ...
  deleteExternal = async (path: string): Promise<void> => {
