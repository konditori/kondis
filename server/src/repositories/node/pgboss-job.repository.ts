...
import { JobCounts, JobHistoryEntry, JobHistoryStatus, JobItem } from 'src/types/jobs';
...
export class PgBossJobRepository extends JobRepository {
  ...
  constructor(
    private readonly config: Pick<ConfigRepository, 'database'>,
    private readonly consumeJobs: boolean,
    private readonly logger: Pick<Logger, 'error' | 'log' | 'verbose' | 'warn'> = new Logger(PgBossJobRepository.name),
  ) {
    super();
  }
