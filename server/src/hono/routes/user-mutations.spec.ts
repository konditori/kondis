import { describe, expect, it, vi } from 'vitest';

import { createHonoApp } from 'src/hono/app';
import { honoAuthHeaders, newHonoDependencies, newHonoUsers, TEST_HONO_USER } from 'test/hono';

describe('Hono user mutation routes', () => {
  it('updates the signed-in user and handles avatar mutations through the upload adapter', async () => {
    const file = { originalname: 'avatar.png', size: 6, buffer: Buffer.from('avatar') };
    const read = vi.fn(() => Promise.resolve(file));
    const updateProfile = vi.fn(() =>
      Promise.resolve({
        id: TEST_HONO_USER.id,
        email: TEST_HONO_USER.email,
        firstName: 'Updated',
        lastName: 'Name',
        role: 'user' as const,
        avatarUrl: null,
      }),
    );
    const uploadAvatar = vi.fn(() => Promise.resolve({ avatarUrl: '/api/v1/users/me/avatar' }));
    const clearAvatar = vi.fn(() => Promise.resolve());
    const app = createHonoApp(
      newHonoDependencies({
        uploads: { read },
        userService: { clearAvatar, updateProfile, uploadAvatar },
        users: newHonoUsers(),
      }),
    );

    const update = await app.request('/users/me', {
      method: 'PATCH',
      headers: { ...honoAuthHeaders(), 'Content-Type': 'application/json' },
      body: JSON.stringify({ firstName: ' Updated ', lastName: ' Name ' }),
    });
    expect(update.status).toBe(200);
    expect(updateProfile).toHaveBeenCalledWith(TEST_HONO_USER.id, 'Updated', 'Name');

    const upload = await app.request('/users/me/avatar', { method: 'POST', headers: honoAuthHeaders() });
    expect(upload.status).toBe(201);
    expect(read).toHaveBeenCalledWith(expect.any(Request), undefined, 'avatar');
    expect(uploadAvatar).toHaveBeenCalledWith(TEST_HONO_USER.id, file);

    const deletion = await app.request('/users/me/avatar', { method: 'DELETE', headers: honoAuthHeaders() });
    expect(deletion.status).toBe(204);
    expect(clearAvatar).toHaveBeenCalledWith(TEST_HONO_USER.id);
  });

  it('keeps user creation restricted to administrators', async () => {
    const create = vi.fn();
    const response = await createHonoApp(newHonoDependencies({ auth: { create }, users: newHonoUsers() })).request(
      '/users',
      {
        method: 'POST',
        headers: { ...honoAuthHeaders(), 'Content-Type': 'application/json' },
        body: JSON.stringify({
          email: 'new@example.com',
          firstName: 'New',
          lastName: 'User',
          password: 'long enough password',
        }),
      },
    );

    expect(response.status).toBe(403);
    expect(create).not.toHaveBeenCalled();
  });
});
