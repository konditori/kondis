...
describe('Hono social mutation routes', () => {
  it('delegates relationship, engagement, notification, and comment mutations', async () => {
    ...
    const comment = {
      ...
      user: { id: TEST_HONO_USER.id, firstName: 'Test', lastName: 'User', avatarUrl: null },
    };
    const service = {
    ...
    const app = createHonoApp(newHonoDependencies({ social: service, users: newHonoUsers() }));
    const request = (path: string, method: string, body?: object) =>
      app.request(path, {
        method,
        body: body ? JSON.stringify(body) : undefined,
        headers: { ...honoAuthHeaders(), ...(body && { 'Content-Type': 'application/json' }) },
      });
    ...
    expect(service.sendRequest).toHaveBeenCalledWith(TEST_HONO_USER.id, TARGET_ID);
    expect(service.like).toHaveBeenNthCalledWith(1, ACTIVITY_ID, TEST_HONO_USER.id, true);
    expect(service.like).toHaveBeenNthCalledWith(2, ACTIVITY_ID, TEST_HONO_USER.id, false);
    expect(service.addComment).toHaveBeenCalledWith(ACTIVITY_ID, TEST_HONO_USER.id, 'Nice run');
    expect(service.updateComment).toHaveBeenCalledWith(ACTIVITY_ID, COMMENT_ID, TEST_HONO_USER.id, 'Updated');
  });
});
