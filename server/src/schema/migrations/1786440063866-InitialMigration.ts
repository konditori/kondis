    ...
    BEGIN
      ...
      WITH efforts AS (
        ...
        SELECT
          ...
          year,
          ROW_NUMBER() OVER (
            PARTITION BY sport, type
            ORDER BY
              CASE WHEN value_kind = 'duration' THEN value END ASC NULLS LAST,
              CASE WHEN value_kind <> 'duration' THEN value END DESC NULLS LAST,
              activity_id
          )::integer AS overall_rank,
          ROW_NUMBER() OVER (
            PARTITION BY sport, type, year
            ORDER BY
              CASE WHEN value_kind = 'duration' THEN value END ASC NULLS LAST,
              CASE WHEN value_kind <> 'duration' THEN value END DESC NULLS LAST,
              activity_id
...
export async function down(db: Kysely<unknown>): Promise<void> {
  ...
  await sql`DROP FUNCTION IF EXISTS kondis_refresh_best_effort_rankings()`.execute(db);
