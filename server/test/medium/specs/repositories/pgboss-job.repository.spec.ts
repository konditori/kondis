...
describe(PgBossJobRepository.name, () => {
  let testApp: TestApp;
  ...
  let storageRepository: FileSystemStorageRepository;
  let ownerId: string;
  ...
  beforeAll(async () => {
    ...
    storageRepository = testApp.get(FileSystemStorageRepository);
  }, 60_000);
  ...
  describe('handler discovery', () => {
    ...
    const buildSamples = async (ownerId: string): Promise<Record<JobName, JobItem>> => ({
      ...
      [JobName.ActivityUpload]: {
        ...
        data: {
          ...
          checksum: await new NodeCryptoRepository().sha256(SAMPLE_GPX),
        },
      },
    ...
    it('binds a handler to every job name', async () => {
      ...
      for (const item of Object.values(await buildSamples(ownerId))) {
        await expect(jobService.execute(item)).resolves.toSatisfy((status) =>
  ...
  describe('end to end', () => {
    it('deletes the activity, the upload and the file', async () => {
      ...
      const uploaded = await uploadRepository.getByChecksum(await new NodeCryptoRepository().sha256(contents));
